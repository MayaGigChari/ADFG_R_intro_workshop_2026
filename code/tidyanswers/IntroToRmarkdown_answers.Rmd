...
Press play to create an object of the first 7 numbers in a Fibonnaci sequence, where each number is the sum of the two preceding ones.
```{r}
fibonacci <- c(1,1,2,3,5,8,13)
```

Notice that you when you run that code it created an object in your global environment!  
...
tail(fibonacci, n=2)  # Show the last two values on the vector 

fibonacci <- c(fibonacci,sum(tail(fibonacci,n=2))) #combines the original vector, with the sum of the last two values,
                                                   #Then saves it as a new vector with the same name
...
Have you noticed how some of the text in this document looks a little different? That's because R Markdown uses special characters to help format your writing. For example:
...
```{r}
fibonacci <- c(1,1) # first two values of the Fibonacci sequence
```
...
# Approximate the golden ratio using the last two numbers
phi_approx <- tail(fibonacci, n = 1) / tail(fibonacci, n = 2)[1]
# phi_approx <- fibonacci[length(fibonacci)] / fibonacci[length(fibonacci) - 1]
