...
When you are satisfied, lets revisit our regression line from earlier. Lets now color our points by species and redraw the regression.
...
# 1. Scatterplot of bill length vs. bill depth, faceted by species
ggplot(penguins, aes(x = bill_length_mm, y = bill_depth_mm)) +
  geom_point(aes(color = sex)) +          # Color points by sex
  facet_wrap(~ species) +                 # One plot per species
  labs(title = "Bill Length vs. Depth by Species")

# 2. Scatterplot of flipper length vs. body mass, faceted by island
ggplot(penguins, aes(x = flipper_length_mm, y = body_mass_g)) +
  geom_point(aes(color = species)) +
  facet_wrap(~ island) +
  labs(title = "Flipper Length vs. Body Mass by Island")

# 3. Boxplot of body mass by species, faceted by sex
ggplot(penguins, aes(x = species, y = body_mass_g)) +
  geom_boxplot(aes(fill = species)) + 
  facet_wrap(~ sex) + 
  labs(title = "Body Mass by Species (Faceted by Sex)")

# 4. Scatterplot with facet_grid: sex (rows) by island (columns)
ggplot(penguins, aes(x = bill_length_mm, y = flipper_length_mm)) +
  geom_point(aes(color = species)) +
  facet_grid(sex ~ island) + 
  labs(title = "Bill vs. Flipper Length (Faceted by Sex and Island)")

# 5. Boxplot of flipper length by species, faceted by island (grid format)
ggplot(penguins, aes(x = species, y = flipper_length_mm)) +
  geom_boxplot(aes(fill = species)) +
  facet_grid(. ~ island) + 
  labs(title = "Flipper Length by Species Across Islands")

# 6. Scatterplot with shape and color, faceted by species and sex
ggplot(penguins, aes(x = bill_depth_mm, y = body_mass_g)) +
  geom_point(aes(color = island, shape = sex)) +
  facet_grid(species ~ sex) +
  labs(title = "Body Mass by Bill Depth (Faceted by Species and Sex)")
